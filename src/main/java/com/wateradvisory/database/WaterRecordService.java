...
public class WaterRecordService {
    ...
    /**
     * Every activity this user has ever logged, flattened out of all their
     * daily_water_records rows (each row stores its activities as an embedded
     * JSON array). Used by the Conservation Tips page to generate real
     * personalised tips instead of hardcoded placeholders.
     *
     * <p>Reuses the SAME Supabase connection / auth as {@link #getUserTotalWater()}
     * -- no new table, permission, or schema change. Like every other read in this
     * class the row filter uses the authenticated session id from
     * {@link UserSession}; the {@code userId} argument mirrors the in-memory
     * {@code WaterDataList} API so call sites have one consistent signature and is
     * logged on failure for debugging.</p>
     *
     * <p>KNOWN LIMITATION: neither daily_water_records nor the embedded activity
     * objects carry a per-activity timestamp (only a day-level {@code record_date}),
     * so no returned entry has a time of day. Time-of-day tips (e.g. "tap usage
     * spikes 7-8am") are therefore NOT possible yet -- blocked on a future
     * {@code created_at} column being added to the activity rows.</p>
     *
     * @return the activity list, or an empty list if not logged in / on any error
     *         (callers must treat empty as "no data yet", never crash).
     */
    public static List<WaterActivityEntry> getUserActivities(int userId) {
        ...
        try {
            ...
            // Not signed in (e.g. running Charlie's screens in isolation): no data.
            if (sessionUserId == null || accessToken == null) {
                return activities;
            }
            ...
            if (response.statusCode() != 200) {
                ...
                System.out.println(
                        "Failed to get activities for user " + userId
                                + ": " + response.body()
                );
            ...
            for (JsonElement rowElement : rows) {
                ...
                for (JsonElement element : savedActivities) {
                    ...
                    activities.add(new WaterActivityEntry(
                            activity.get("activity").getAsString(),
                            activity.get("duration").getAsInt(),
                            activity.get("amount").getAsInt(),
                            activity.get("water_litres").getAsDouble()
